...
public:
	UploadManager(boost::asio::io_service& io_service, size_t transmission_unit,
				  ...
				  bool use_proxy, std::string proxy_address,
				  uint8_t connections, const std::string& upload_dir)
	: io_service(io_service)
	, watcher(io_service, transmission_unit, server_ip, server_port, use_proxy, proxy_address, upload_dir)
    {
...
int main(int argc, char** argv)
{
	...
	boost::asio::io_service io_service;
	UploadManager manager(io_service, transmission_unit, server_address, server_port,
						  use_proxy, proxy_address, connections, upload_path);
    io_service.run();
    return 0;
}
